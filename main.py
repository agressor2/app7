...
def equation(A, b, c, eps):
    ...
    b = b[max_diagonal_prod[1]]
    print(b)
    if det(A) == 0:
        return "Нет корней", "Нет корней", "Нет корней", "Нет корней"
    condition = True
    variables0 = []
    while condition:
        variables0 = c.copy()
        ...
        if max(abs(c[i] - variables0[i]) for i in range(len(c))) <= eps or str(c[0]) == 'nan':
            # print([abs(c[i] - variables0[i]) for i in range(len(c))])
            condition = False
        # break
    if str(c[0]) == 'nan':
        return 'Привышено максимальное значение', 'Привышено максимальное значение', 'Привышено максимальное значение', 'Привышено максимальное значение'
    return [str(c[i]) for i in range(len(c))]
